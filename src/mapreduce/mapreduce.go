...
type MapReduce struct {
  ...
  // idleWorkersMapChannel chan bool // 用做读写IdleWorkers的同步channel
  idleWorkersListChannel chan bool
  idleWorkersLock sync.Mutex
  // IdleWorkers map[string]*WorkerInfo //空闲可用的worker
...
func InitMapReduce(nmap int, nreduce int,
  ...
  // mr.idleWorkersMapChannel = make(chan bool)
  mr.DoneChannel = make(chan bool)

  // initialize any additional state here
  mr.idleWorkersListChannel = make(chan bool, 10)
  mr.IdleWorkersList = list.New()
  return mr
}
...
func (mr *MapReduce) Register(args *RegisterArgs, res *RegisterReply) error {
  ...
  fmt.Println("waiting for idle lock\n")
  mr.idleWorkersLock.Lock()
  //mr.Workers[args.Worker].address = args.Worker
  fmt.Printf("args.worker: %s\n", args.Worker)
  mr.IdleWorkersList.PushBack(args.Worker)
